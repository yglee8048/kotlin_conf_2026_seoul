...
import com.example.demo.config.AsyncConfig.Companion.HOME_INFO_TASK_EXECUTOR
import com.example.demo.config.AsyncConfig.Companion.OPEN_BANKING_TASK_EXECUTOR
 ...
 *    각각 전용 Executor 를 쓴다. (200ms + 500ms = 700ms  ->  max(200ms, 500ms) = 500ms)
 * 2. 홈 화면 접속 기록 적재는 응답에 쓰이지 않으므로 `@Async` 로 **fire-and-forget** 한다.
...
class HomeItemServiceV2(
    ...
    fun getHomeItemsV2(userId: UserId): List<HomeItem> {
        ...
        // 두 Future 는 이미 실행 중이므로, 각각 join() 하는 것만으로 대기 시간은 max(200ms, 500ms) 다.
        // 하나가 실패하면 CompletionException 이 던져지고, 나머지 하나는 취소되지 않고 계속 실행된다.
